...
# for PSS
# simulation function
simulation_PSS <- function(times,n,data,n.include,seed=1){
  # get data
  data.mi <- data
...
# for pss
# reverse
data.mi$perceived_stress_sca_4 <- 4- data.mi$perceived_stress_sca_4
data.mi$perceived_stress_sca_5 <- 4- data.mi$perceived_stress_sca_5
data.mi$perceived_stress_sca_7 <- 4- data.mi$perceived_stress_sca_7
data.mi$perceived_stress_sca_8 <- 4- data.mi$perceived_stress_sca_8

# extract parameters
par.cle <- invariance_alignment_cfa_config(dat = data.mi[,pss.vars], 
                                           group = data.mi$UserLanguage)
...
now <- foreach (i = seq(1,times)) %dopar%{
  # get result
  simulation_PSS(1,100,data.mi,28,i)
  #  message(sprintf('%d',i))
}
end_100<-Sys.time()
elapsed_100 <- end_100 - start_100
...
# save n = 100
write.csv(data.frame(simulate_100),file='simulate_100_pss.csv',row.names = F)
...
now <- foreach (i = seq(1,times)) %dopar%{
  # get result
  simulation_PSS(1,200,data.mi,28,i)
  #  message(sprintf('%d',i))
}
end_200<-Sys.time()
elapsed_200 <- end_200 - start_200
...
# save n = 200
write.csv(data.frame(simulate_200),file='simulate_200_pss.csv',row.names = F)

# n= 500
start_500 <-Sys.time()
now <- foreach (i = seq(1,times)) %dopar%{
  # get result
  simulation_PSS(1,500,data.mi,28,i)
  #  message(sprintf('%d',i))
}
end_500<-Sys.time()
elapsed_500 <- end_500 - start_500
...
# save n = 200
write.csv(data.frame(simulate_500),file='simulate_500_pss.csv',row.names = F)
# stop cluster
parallel::stopCluster(cl)
